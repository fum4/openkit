When the user mentions an issue key (like PROJ-123, ENG-42, LOCAL-1), a ticket number, or says "work on <something>", use the dawg CLI (not MCP) from the project root.

## Source Resolution (CLI-first)

1. Resolve source deterministically first:
   - `dawg task resolve <ID> --json`
2. The resolver checks in this order:
   - Existing issue files under `.dawg/issues/` (local/jira/linear)
   - Connected integrations in `.dawg/integrations.json`
...
4. When a prefixed key is validated for Jira/Linear (e.g. `ABC-123`), dawg updates the corresponding default key in `.dawg/integrations.json` automatically.

## Run Task

- Preferred non-interactive command:
  - `dawg task <ID> --init` (auto-resolve source)
- Explicit source fallback:
  - `dawg task jira <ID> --init`
  - `dawg task linear <ID> --init`
  - `dawg task local <ID> --init`
...
1. Read task context:
   - Primary: `.dawg/issues/<source>/<issue-id>/task.json` (or `issue.json` for jira/linear)
   - Notes/todos: `.dawg/issues/<source>/<issue-id>/notes.json`
   - Optional generated context file in worktree: `TASK.md`
2. Read hook configuration from `.dawg/hooks.json`.
