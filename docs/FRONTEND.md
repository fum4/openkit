...
- Attaches interactive WebSocket streams at `/_ok/mobile/v1/agent-sessions/:sessionId/ws`.
- Renders terminal output using an embedded xterm.js WebView.
- Exposes native mobile control keys (`Esc`, `Tab`, arrows, `Ctrl+C`, `Ctrl+D`) to support agent interactive choice UIs.
- Maintains an app-level WebSocket heartbeat to reduce idle disconnects on mobile networks.
- Supports concurrent desktop + mobile attachment to the same server-side PTY session.
- Handles deep links at `/connect` (e.g. `mobileapp://connect?origin=...&token=...`) so mobile handoff links can open the app and auto-connect.
