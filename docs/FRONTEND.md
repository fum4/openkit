...
- **Delete workflow** -- delete is non-optimistic: UI cache/tab cleanup runs only after successful `DELETE /api/worktrees/:id` response. On failure, selection/tabs/sessions are preserved and an explicit error is shown.
