...
Shows Jira issue details: summary, description (rendered as Markdown from Atlassian Document Format), status, priority, assignee, and comments. Provides both "Create Worktree" and a split "Code with ..." action that launches Claude, Codex, Gemini CLI, or OpenCode. The last-selected coding agent is reused as the default action across issue/task panels. Manual launches perform a CLI preflight check (`claude`, `codex`, `gemini`, or `opencode`) and show a Homebrew install modal when missing. A shared permissions dialog is used for all supported agents before launch.

### LinearDetailPanel (`LinearDetailPanel.tsx`)

Similar to JiraDetailPanel but for Linear issues. Shows title, description, state, priority, assignee, labels, and attachment previews/download links via a Linear attachment proxy endpoint to avoid auth/CORS failures. Also supports split "Code with ..." launch (Claude, Codex, Gemini CLI, or OpenCode); launches check CLI availability first and offer Homebrew install if missing, and all supported agents use the shared manual permissions choice modal.
...
- `terminal-session` -- terminal lifecycle events (`created`/`closed`) dispatched as `OpenKit:terminal-session` for live agent tab sync

On connection error, the hook closes the stream and retries a worktree fetch after 5 seconds.
...
For agent scopes, desktop resume behavior does not create shell-only placeholder sessions when no scoped agent session exists.
`DetailPanel` performs an initial active-session fetch (`/api/worktrees/:id/terminals/active`) for the selected worktree, then relies on `terminal-session` SSE events for live updates when sessions are started/stopped remotely (for example from mobile), without manual refresh.
...
- **Wi-Fi button** (left of the Settings icon) is a toggle.
- If tunnel is **enabled**, clicking Wi-Fi calls `/api/ngrok/tunnel/disable`.
- If tunnel is **disabled**, clicking Wi-Fi creates a fresh pairing session via `/api/ngrok/pairing/start` and opens the pairing QR modal.
- The pairing start flow ensures the ngrok tunnel exists and starts it when needed.
- The QR modal renders `mobilePairUrl` as the QR payload (camera-safe HTTPS handoff URL that redirects to the app deep link) and still displays the browser pairing URL for fallback/manual flows.
- The `mobilePairUrl` handoff page attempts to open the app deep link automatically and provides a manual "Open App" fallback button for browsers that block automatic custom-scheme launches.
- The QR modal supports copy-to-clipboard for the mobile camera link and tunnel URL regeneration.
- While the QR modal is open, desktop polls `/api/ngrok/pairing/status/:pairingId`:
  - closes automatically when status becomes `used`
  - auto-regenerates a fresh QR token when status becomes `expired` without forcing tunnel URL rotation
- Through ngrok hostnames, only ngrok-specific routes are exposed; pre-existing app routes are blocked by server middleware.

### Mobile App Agent Bridge

`apps/mobile-app` supports in-app scanning and deep-link pairing for remote agent sessions:

- Scans the QR payload generated by `/api/ngrok/pairing/start` (`https://.../_ok/mobile/connect?token=...`), and also accepts `mobileapp://connect?...` and legacy `/_ok/pair` URLs.
- In-app scanner immediately forwards parsed `origin` + `token` into the deep-link connect screen so the full worktree/scope/session flow starts without manual navigation.
- Pairing exchange is one-time per deep-link token; the connect screen guards against repeated exchanges of the same token to avoid accidental rate-limit blocks.
- Server pairing exchange tolerates short duplicate replays from the same client (deep-link/browser double-open) so mobile pairing remains robust.
- Parses `origin` + token from supported QR URLs.
- Exchanges that token through `POST /api/ngrok/pairing/exchange` to obtain a short-lived bearer session.
- Refreshes bearer sessions via `POST /_ok/refresh` before expiry so mobile sessions can stay alive long-term while connected.
- Loads dedicated mobile gateway context (`/_ok/mobile/v1/context`) and project worktrees (`/_ok/mobile/v1/worktrees`).
- Lists scoped agent sessions per worktree (`/_ok/mobile/v1/agent-sessions`) for `claude|codex|gemini|opencode`.
- Connects or starts scoped sessions through `POST /_ok/mobile/v1/agent-sessions/connect`.
- Mobile starts agent scopes in interactive mode; when a new scoped session is created and the user provided a start prompt, the app injects that prompt after WebSocket attach (instead of relying on one-shot CLI prompt arguments that can auto-exit).
- Attaches interactive WebSocket streams at `/_ok/mobile/v1/agent-sessions/:sessionId/ws` (Bearer auth header plus `accessToken` query fallback for mobile WebSocket compatibility).
- Renders terminal output using an embedded xterm.js WebView (ANSI/VT handling, cursor state, alternate-screen interactive UIs).
- Sends terminal input from mobile (including xterm keyboard events) and receives live output for continuation of laptop-started agent sessions.
- Opens terminal interaction in a bottom-sheet panel after session connect, with a right-side vertical floating action rail (`Submit`, `Esc`, navigation/control keys, `Hide`) for quick actions.
- Exposes native mobile control keys (`Esc`, `Tab`, arrows, `Ctrl+C`, `Ctrl+D`) to support agent interactive choice UIs.
- Retries transient terminal WebSocket disconnects automatically and exposes a dedicated "Hide Keyboard" action for mobile input control.
- Maintains an app-level WebSocket heartbeat (`ping`/`pong`) to reduce idle disconnects on mobile networks and ngrok hops.
- On reconnect, mobile performs full recovery (refresh auth when needed, re-resolve scoped session, and `startIfMissing=true` create-or-attach) instead of only retrying the same stale session id.
- For `unauthenticated` and `session-not-found` WebSocket closes, mobile auto-recovers by refreshing/re-resolving session state before reattach.
- Supports concurrent desktop + mobile attachment to the same server-side PTY session.
- Shows an in-app terminal debug trace (recent WS lifecycle events, close codes/reasons, retry scheduling) to simplify diagnosing disconnects like early `1006/404`.
- Handles deep links at `/connect` (for example `mobileapp://connect?origin=...&token=...`) so mobile handoff links can open the app and auto-connect.
...
| `TabBar.tsx`                | Electron multi-project tab bar and bottom-right project controls (Wi-Fi pairing action and Settings)                                                                                                                                                                                        |
