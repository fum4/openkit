...
- **Tab bar** -- Logs | Terminal | Hooks, plus Claude/Codex/Gemini/OpenCode controls. Each agent appears as its own tab only when opened; otherwise `+ Claude`, `+ Codex`, `+ Gemini`, and `+ OpenCode` quick actions are shown. From the worktree detail panel, `+ Claude` and `+ Codex` are restore-first actions: they prefer a live scoped PTY session, then native agent history filtered to the active worktree's safe current/latest cwd, show a picker when multiple saved conversations match, and only offer `Start New Conversation` as an explicit follow-up when no restore target exists. `+ Gemini` and `+ OpenCode` keep the existing live-session launch-or-resume behavior.
...
- **TerminalView** -- interactive xterm.js terminal. Sessions are reused per project-server+worktree+scope (`terminal`/`claude`/`codex`/`gemini`/`opencode`), so identical worktree IDs in different projects do not collide. Sessions with startup commands (agent launch) are bootstrapped server-side and later reattached in the UI, including after full page refresh via active-session lookup. On reattach, the server now sends a serialized terminal-state snapshot first so worktree terminals restore the current screen plus bounded scrollback instead of relying on raw PTY output replay alone. Worktree-detail `Claude` and `Codex` quick actions can also bootstrap from native history when no live scoped PTY exists, but only from sessions whose safe current/latest cwd belongs to the active worktree. Codex history resume is launched as `codex -C <worktree.path> resume <sessionId>` so same-named worktrees resume without an interactive cwd prompt; Claude continues to use `claude -r <sessionId>` and relies on the stricter restore filter. Agent launch requests are one-shot and scope-tagged: once handled (`reattached`, `started`, or `failed`), launch intent is cleared in both tab-local state and app-level runtime scope state, and requests whose scope does not match the active project are dropped before they reach `TerminalView`. Explicit launch requests run a scoped reconcile connect path that bypasses client SID cache so the server can reuse an existing agent session or replace a shell-only scoped session as needed. Passive reconnect for agent tabs is server-authoritative: on project switches and watchdog retries, `useTerminal` asks the active backend for the current scoped session before trusting client cache, updates any stale cached SID to match the server, and reattaches non-destructively. When explicit launch intent is present, passive reconnect is suppressed; if a passive connect is already in progress, explicit launch retries in a short bounded loop instead of being marked failed immediately. If no active scoped agent session exists during passive reconnect, the client records a passive-reconnect miss instead of creating a fresh shell-only scoped session. Awaiting-input notifications are explicit agent events via `openkit activity await-input`, not terminal-text heuristics. Closing agent tabs explicitly destroys their scoped sessions. Expected client-side websocket closes caused by cleanup, React DEV remounts, or superseded connect generations no longer force destructive scoped-session refreshes.
...
- Launch requests are consumed after first handling, cleared from both app-level and detail-panel transient state, and are not restored across project switches; returning focus to a tab only does passive reattach.
...
Sessions are keyed by `runtimeScopeKey + worktreeId + scope` (`terminal`, `claude`, `codex`, `gemini`, or `opencode`). `runtimeScopeKey` is `project:<id>` in Electron and `server:<url-or-relative>` in web mode. `TerminalView` keeps sessions alive across tab switches/navigation, and visible tabs automatically reconnect when project/server context changes. Reconnect is reattach-first; startup commands are only used while a one-shot explicit launch request is pending. The server tracks terminal state with a headless xterm instance and serializes up to 10,000 lines of scrollback for restore-on-reattach across all scopes using `@xterm/headless` with proposed API enabled for `SerializeAddon`. Explicit launch intent takes precedence over passive reconnect attempts, and in-flight connect contention is handled by bounded explicit-launch retries instead of immediate failure. For agent scopes, passive reconnect is server-authoritative and non-creating: reconnect first resolves the backend's active scoped session, then optionally falls back to the client cache if the backend reports none, and it never creates a new shell session simply because a project switch interrupted websocket attach. In Electron mode, `serverUrl` follows the actual runtime port published by the desktop app after it validates the spawned server's `.openkit/server.json`, so frontend fetches and SSE connections follow server-side port bumps automatically. To prevent infinite reconnect loops, websocket open attempts have a timeout, rapid repeated open->close cycles can still force a fresh session for plain terminals, and visible reconnecting tabs run a watchdog that retries agent reattach against the backend's authoritative scoped session before escalating.
...
| `activityFeedUtils.ts`         | Shared activity feed upsert/history/hook-aggregation utilities                                                                     |
