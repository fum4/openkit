...
OpsLog (backend)
  ...
  ├─ Stores fetch-monitor outbound HTTP events (including request/response payload metadata when readable)
  ├─ Stores internal lifecycle events (task, terminal, worktree delete phases)
...
| `apps/server/src/runtime/fetch-monitor.ts`            | Runtime outbound fetch monitor feeding `OpsLog`                                                                  |
