...
- **Restore snapshots**: Each session mirrors PTY output into a headless xterm instance with bounded scrollback, serializes that state on reattach, and sends a restore frame before live output resumes. The headless terminal enables proposed API support because `SerializeAddon` depends on it.
...
- **Server-authoritative passive reattach**: Passive reconnect for scoped agent tabs is driven by the backend's current scoped-session mapping, not by the renderer's cached terminal SID. On project switches and reconnect watchdog retries, the client first resolves `GET /api/worktrees/:id/terminals/active?scope=...`, reconciles any stale cache entry to the backend answer, and only tries a cached SID directly when the backend reports no active scoped session. Passive reconnect misses are non-destructive and do not create a replacement shell session for agent tabs.
- **Runtime scope restore**: Renderer runtime state is scoped by project/server identity, but active launch requests are treated as one-shot transient state. Scope restoration brings back passive tab/session state and durable pending queues only; already-promoted launch requests are dropped instead of being replayed after a project switch. Active requests are stamped with their originating scope key, and detail-panel-local transient launch state is cleared on scope changes and after handling so same-named worktrees cannot replay another project's launch request.
- **Electron runtime port publication**: The desktop app requests a preferred port for each project, but the embedded server may bind a different port if the preferred one is already in use. Electron reconciles this by reading `.openkit/server.json`, validating the PID against the spawned child process, probing the discovered URL for readiness, and publishing the actual bound port to the renderer as `Project.port`.
- **Historical agent restore**: Terminal routes also expose restore discovery for worktree-detail `claude` / `codex` quick actions by combining live scoped PTY lookup with native agent history filtered by safe current/latest cwd. Claude history is only accepted when the latest recorded cwd equals the active worktree path; Codex history remains exact-cwd and resumes with an explicit `-C <worktree.path>` override.
