...
export function getTerminalWsUrl(sessionId: string, serverUrl: string | null = null): string {
  if (serverUrl) {
    // Convert the current HTTP server URL to a WS URL
...
export async function fetchNgrokConnectStatus(
  serverUrl: string | null = null,
): Promise<NgrokConnectStatus> {
  try {
    const res = await fetch(`${getBaseUrl(serverUrl)}/api/ngrok/status`);
    if (!res.ok) throw new Error("Failed to fetch ngrok connect status");
    return await res.json();
  } catch (err) {
    return {
      success: false,
      ...
      error: err instanceof Error ? err.message : "Failed to fetch ngrok connect status",
    };
  }
}

export async function enableNgrokTunnel(
  regenerateUrl = false,
  serverUrl: string | null = null,
): Promise<NgrokConnectStatus> {
  try {
    const res = await fetch(`${getBaseUrl(serverUrl)}/api/ngrok/tunnel/enable`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ regenerateUrl }),
    });
    if (!res.ok) throw new Error("Failed to enable ngrok tunnel");
    return await res.json();
  } catch (err) {
    return {
      success: false,
      ...
      tunnel: {
        ...
        localPort: null,
        startedAt: null,
        error: err instanceof Error ? err.message : "Failed to enable ngrok tunnel",
      },
      error: err instanceof Error ? err.message : "Failed to enable ngrok tunnel",
    };
  }
...
export async function disableNgrokTunnel(
  serverUrl: string | null = null,
): Promise<NgrokConnectStatus> {
  try {
    const res = await fetch(`${getBaseUrl(serverUrl)}/api/ngrok/tunnel/disable`, {
      method: "POST",
    });
    if (!res.ok) throw new Error("Failed to disable ngrok tunnel");
    return await res.json();
  } catch (err) {
    return {
      success: false,
      ...
      tunnel: {
        ...
        localPort: null,
        startedAt: null,
        error: err instanceof Error ? err.message : "Failed to disable ngrok tunnel",
      },
      error: err instanceof Error ? err.message : "Failed to disable ngrok tunnel",
    };
  }
...
export async function createNgrokPairingSession(
  regenerateUrl = false,
  next = "/",
  serverUrl: string | null = null,
): Promise<NgrokPairingStartResponse> {
  try {
    const res = await fetch(`${getBaseUrl(serverUrl)}/api/ngrok/pairing/start`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ regenerateUrl, next }),
    });
    return await res.json();
  } catch (err) {
    return {
